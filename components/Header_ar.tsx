...
const Header_ar: React.FC<{ onToggleLanguage: () => void; onShowFormulas?: () => void }> = ({ onToggleLanguage, onShowFormulas }) => {
  return (
    <motion.header 
          ...
          <div className="flex items-center gap-2">
              {onShowFormulas && (
                ...
                >
                  ...
                  <span className="hidden sm:inline">طرق الاحتساب والمعادلات</span>
                  <span className="sm:hidden">المعادلات</span>
                </button>
              )}
              <button 
                onClick={onToggleLanguage}
                className="px-4 py-2 text-xs sm:text-sm font-medium text-[#1D1D1F] bg-gray-100/50 hover:bg-gray-100 rounded-full transition-all active:scale-95 border border-transparent hover:border-gray-200 font-sans"
              >
                English
              </button>
          </div>
